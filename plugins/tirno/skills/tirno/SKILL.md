---
name: tirno
description: tirno CLI 명령 레퍼런스 — raw CDP 위의 멀티세션 브라우저 자동화. 세션 관리(new/restart/ls/attach/kill/gc), 앵커, 내비게이션·입력·조회(nav/click/fill/snapshot/screenshot/eval), 진단(stall/audit/trace/screencast), 캐시·trail, 그리고 기계용 자기기술(schema). 브라우저 세션을 띄우거나 CDP 로 페이지를 조작·조회하려 할 때 참고. 작업 절차는 tirno:tirno-runbook, chrome-devtools-mcp 대체는 tirno:tirno-mcp.
---
...
tirno new sandweb -- --no-proxy-server --ignore-certificate-errors \
  --host-resolver-rules="MAP *.imtest.me 127.0.0.1"
