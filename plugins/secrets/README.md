...
| `secret-get --prime <name>` | 둘 다 | 복호화 성공만 확인, **비밀값은 출력하지 않음** — 사람은 비밀번호를 화면에 띄우지 않고 캐시 채우기, Claude 는 대화 노출 없이 캐시 생존 확인 | 캐시 있으면 `OK` / 없으면 실패 + 안내 |
| `secret-list` | Claude 자율 호출 가능 | 등록된 service-name 목록 (복호화 없음 = 비밀값 비노출, passphrase 불필요) | 항상 동작 |

## 하루 한 번 passphrase, 12시간 자유 조회

```
사람: 자기 터미널에서 secret-get --prime <name> 실행 → pinentry 로 passphrase 입력
      (--prime: 캐시만 채우고 비밀번호는 화면에 출력하지 않는다)
                     │
                     ▼
